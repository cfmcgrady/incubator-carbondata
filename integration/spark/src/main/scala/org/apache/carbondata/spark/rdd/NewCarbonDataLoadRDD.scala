  ...
  override def compute(theSplit: Partition, context: TaskContext): Iterator[(K, V)] = {
    ...
    val iter = new Iterator[(K, V)] {
      ...
      try {
        ...
        val recordIterator: java.util.Iterator[Array[AnyRef]] = new NewRddIterator(
          firstParent[Row].iterator(theSplit, context),
          carbonLoadModel)

        val recordIterators = Array(recordIterator)
        ...
        loadMetadataDetails.setLoadStatus(CarbonCommonConstants.STORE_LOADSTATUS_SUCCESS)
        new DataLoadExecutor().execute(model, loader.storeLocation, recordIterators)
